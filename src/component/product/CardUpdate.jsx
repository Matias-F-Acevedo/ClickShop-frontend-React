            ...
            <div>
                <div className="card-actualizar">
                    <form className="form-update" onSubmit={(e) => actualizarProducto(e)}>
                        ...
                        <label>Nombre</label>
                        <input type="text" name="date" placeholder="Ubicacion" minLength={"4"} value={currentProduct.product_name} onChange={(e) => { setCurrentProduct(prev => ({ ...prev, product_name: e.target.value })) }} required />
                    ...
                    <div className="container-btn-confirmar-cancelar">
                        <button className="btn-eliminar" onClick={() => setUpdate(false)}>Cancelar</button>
                    </div>
                </div>
            </div>
        </>
